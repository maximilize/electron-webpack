...
class DevRunner {
  async start() {
    ...
    const env = Object.assign(Object.assign({}, getCommonEnv()), {
      ELECTRON_WEBPACK_WDS_HOST: wdsHost,
      ELECTRON_WEBPACK_WDS_PORT: wdsPort
    });
    const hmrServer = new HmrServer();
    await Promise.all([startRenderer(projectDir, env), hmrServer.listen().then(it => {
      socketPath = it;
    ...
    args.push(path.join(electronWebpackConfig.commonDistDirectory, "main/main.js")); // Pass remaining arguments to the application. Remove 3 instead of 2, to remove the `dev` argument as well.
